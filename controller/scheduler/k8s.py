...
class KubeHTTPClient():
    ...
    def create(self, name, image, command, **kwargs):
        ...
        if self._get_rc(name) == 200 :
            self.scale(name, image,kwargs.get('num', {}))
            return
    ...
    def _create_service(self,name,app_name):
      ...
      for _ in xrange(300):
          ...
          conn_pod.request('GET','/api/'+self.apiversion+'/namespaces/default/pods')
          ...
          conn_pod.close()
          if not 200 <= status <= 299:
              errmsg = "Failed to get Pods for Replication Controller:{} {} {} - {}".format(
                  name,status, reason, data)
              raise RuntimeError(errmsg)
          parsed_json =  json.loads(data)
