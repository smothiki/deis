...
class App(UuidAuditedModel):
    ...
    def deploy(self, user, release):
        ...
        for e in existing:
            ...
            n.save()
            new.append(n)
            if e.type in scale_types :
                scale_types[e.type] = scale_types[e.type]+1
            else:
                scale_types[e.type] = 1
                old_name = e._job_id

        if new and "deploy" in dir(self._scheduler) :
            self._deploy_app(scale_types,release,old_name,existing)
        else :
    ...
    def _deploy_app(self,scale_types,release,old_name,existing) :
        for scale_type in scale_types :
            image = release.image
            kwargs = {'memory': release.config.memory,
                      'cpu': release.config.cpu,
